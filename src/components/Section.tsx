...
const Section: React.FC<{
    ...
    const getAlignmentClasses = () => {
        if (alignCenterLeft) return 'left-24 top-1/2 -translate-y-1/2';
        if (alignBottomRight) return 'right-24 bottom-24 items-end text-right';
        if (alignRight) return 'right-24 top-24 items-end text-right';
    ...
    return (
        <section ref={sectionRef} className="relative h-screen overflow-hidden">
            ...
            <div
                className={`absolute ${getAlignmentClasses()} flex flex-col gap-4 text-white max-w-[600px] text-shadow`}
            >
